...
public:
	...
	template<typename U>
	void Convert(const IppImage<U>& img, bool useLimit);
	
	// 영상 정보 반환
	int GetWidth() const { return width; }
	int GetHeight() const { return height; }
	int GetSize() const { return width * height; }
	int IsValid() const { return (pixels != NULL); }
};
...
template<typename T> inline T limit(const T& valse);
...
void IppImage<T>::Convert(const IppImage<U>& img, bool useLimit) {
